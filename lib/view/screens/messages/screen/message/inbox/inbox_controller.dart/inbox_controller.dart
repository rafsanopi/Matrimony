...
//Pick File and Select
  void openFile() async {
    ...
    if (result != null) {
      ...
      debugPrint("Picked File =============$pickedFile");
      update();
    }
  }
  ...
  sendMsg(String receiver) async {
    ...
    if (sendMsgController.text.isNotEmpty ||
      ...
      if (response.statusCode == 201) {
        ...
        messageController.readMessage(chatID: chatID);
        ...
        TostMessage.toastMessage(
            message: "${decodeMSG["message"]} upgrade account");
        Get.toNamed(AppRoute.premiumPackages);
        //TostMessage.toastMessage(message: response.statusCode.toString());
      } else {}
      sendMsgController.clear();
      isSending = false;
      update();
  ...
  getInboxChatList() async {
    ...
    if (inboxChatResponse.statusCode == 200) {
      ...
      if (rawData != null) {
        ...
        debugPrint("InboxChat===========$inboxChat");
