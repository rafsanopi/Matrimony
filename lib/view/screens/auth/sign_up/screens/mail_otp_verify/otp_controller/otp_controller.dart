  ...
  Future<void> verifySignUpOtp() async {
    ...
    if (responseModel.statusCode == 200) {
      ...
      AuthModel signUpModel =
          AuthModel.fromJson(jsonDecode(responseModel.responseJson));
      ...
      String phoneNumber =
          signUpModel.data!.attributes!.user!.phoneNumber.toString();

      bool freeSubscription =
          signUpModel.data!.attributes!.user!.freeSubscription!;
      ...
      await prefs.setBool(
          SharedPreferenceHelper.freeSubscription, freeSubscription);

      if (kDebugMode) {
        print(
            "This is Token===========>>>>>>>>>>>>>>>${prefs.get(SharedPreferenceHelper.token)}");
        print(
            "This is ID================>>>>>>>>>>>>>>>${prefs.get(SharedPreferenceHelper.userIdKey)}");
      }
      TostMessage.toastMessage(message: responseModel.message);

      Get.offAllNamed(AppRoute.otpVerifyPhn, arguments: phoneNumber);
    } else {
      TostMessage.toastMessage(message: responseModel.message);
    }
    isLoading = false;
